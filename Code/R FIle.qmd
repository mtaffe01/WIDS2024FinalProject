...
map_data <- full_state_data |> 
  select(c("year", "state", "state_abbreviation", "party_state_avaerage")) |> 
  distinct(year, state, .keep_all = TRUE) |> 
  filter(year %% 4==0) |> 
  filter(!is.na(party_state_avaerage))


plot_usmap(data = map_data, values = "party_state_avaerage", color = "black") +
...
```{r}
state_minimum_wage_data <- full_state_data |> 
