...
full_ntl_data |> 
  ...
  labs(
    ...
    subtitle = "Using incomes adjusted to 2020 rates",
    x = "Year",
...
rm(map_data)
```
We can see that, especially in the northern-midwest states, that the country has grown to be more republican in he last 30 years. However, in the last 10 years this has begun to shift again. During this whole time, it appears that New England has become more democrat.
...
This graph is incredibly interesting. First, a quick view of this grpah shows the exact opposite of our hypothesis. Looking at each decade we can see collective bargaining rates decrease and wage increase. Then, in each decade that relationship continues. Still, we can see that in every decade, a collective bargaining agreement will lead to an increase in wages for a worker over a non-collective-bargaining-jobs. 
...
full_state_data |> 
  ...
  labs(
    ...
    y = "Proportion of House Delegation that is Democration"
  )
```
