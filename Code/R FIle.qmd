...
```
Our first graph shows what is commonly known about unions in the United States: Union membership has been on the decline since at least the 1980s. Unfortunately our data does not extend to the last two years where some reports indicate union memberships are on the rise. Early on, we can see that southern and more Republican leading states have lower union rates than more democrat and northern / western states. There is a very small bump in membership in 2008, which we hypothesize happened in relation to the 2008 financial crisis.
...
```
This graph is almost the same exact as the first, although the rates are slightly higher because this graph includes all collective bargaining agreements.
...
state_minimum_wage_data |> # use created table here
  ...
  geom_line(data = subset(state_minimum_wage_data, # use created object here
            ...
            linewidth = .75) +
  geom_line(aes(y = fed_min_wage_2020_adj), color = "red", linewidth = .75) + # federal minimum wage
  theme_minimal() +
...
effective_minimum_wage_data |> 
  ...
  geom_line(data = subset(effective_minimum_wage_data, !is.na(map_color)),
            ...
            linewidth = .75) +
  geom_line(aes(y = fed_min_wage_2020_adj), color = "red", linewidth = .75) +
  theme_minimal() +
...
```
This graph removes all data points where the state minimum wage falls below the federal amount. From this we can see that more and more states are taking it into their own hands to protect workers rights. We can see a large jump in 2007 which may be related to the financial crisis.
...
full_ntl_data |> 
  ...
  labs(
    ...
    subtitle = "Using mean (red) and median (blue) incomes adjusted to 2020 rates",
    x = "Year",
    y = "Income in USD (Adjusted for Inflation to 2020 Rates"
  )
```
...
full_ntl_data |> 
  ...
  labs(
    title = "Mean and Median Income in the United States Between 1984 and 2019",
    subtitle = "Using mean (red) and median (blue) incomes",
    x = "Year",
    y = "Income in USD"
  )
```

The two above graphs show that when accounting for inflation, Americans are earning less per year now than they did 30 years ago. 
...
rm(map_data)
```
We can see that, especially in the northern-midwest states, that the country has grown to be more republican in the last 30 years. However, in the last 10 years this has begun to shift again. During this whole time, it appears that New England has become more democrat.
...
full_ntl_data |> 
  ...
  geom_point(aes(y = wage_2020_adj), color = "blue") +
  geom_jitter(aes(y = union_wage_2020_adj), color = "red") +
  facet_wrap(~decade) 
```

This graph is incredibly interesting. First, a quick view of this graph shows the exact opposite of our hypothesis. Looking at each decade we can see collective bargaining rates decrease and wage increase. Then, in each decade that relationship continues. Still, we can see that in every decade, a collective bargaining agreement will lead to an increase in wages for a worker over a non-collective-bargaining-jobs. 
...
This graph demonstrates the same as the previous graph, but with a linear regression line for each region.
...
full_state_data |> 
  ...
  filter(year %% 4==0) |>
  ggplot(aes(x = percent_collective_bargain, y = party_state_average)) +
  geom_jitter(aes(color = regions), alpha = 0.1, width = 0.1, height = 0.1) +
...
```{r}
# 1984 to 2019
t.test(full_ntl_data$union_wage_2020_adj, full_ntl_data$nonunion_wage_2020_adj)

# 1980s
full_ntl_data_1980s <- full_ntl_data |>
  filter(year >= 1984 & year < 1990)
t.test(full_ntl_data_1980s$union_wage_2020_adj, full_ntl_data_1980s$nonunion_wage_2020_adj)

# 1990s
full_ntl_data_1990s <- full_ntl_data |>
  filter(year >= 1990 & year < 2000)
t.test(full_ntl_data_1990s$union_wage_2020_adj, full_ntl_data_1990s$nonunion_wage_2020_adj)

# 2000s
full_ntl_data_2000s <- full_ntl_data |>
  filter(year >= 2000 & year < 2010)
t.test(full_ntl_data_2000s$union_wage_2020_adj, full_ntl_data_2000s$nonunion_wage_2020_adj)

# 2010s
full_ntl_data_2010s <- full_ntl_data |>
  filter(year >= 2010 & year < 2020)
t.test(full_ntl_data_2010s$union_wage_2020_adj, full_ntl_data_2010s$nonunion_wage_2020_adj)
```

T test between median and mean incomes
```{r}
# 1984 to 2019
t.test(full_ntl_data$median_income_2020_adj, full_ntl_data$mean_income_2020_adj)

# 1980s
full_ntl_data_1980s <- full_ntl_data |>
  filter(year >= 1984 & year < 1990)
t.test(full_ntl_data_1980s$median_income_2020_adj, full_ntl_data_1980s$mean_income_2020_adj)

# 1990s
full_ntl_data_1990s <- full_ntl_data |>
  filter(year >= 1990 & year < 2000)
t.test(full_ntl_data_1990s$median_income_2020_adj, full_ntl_data_1990s$mean_income_2020_adj)

# 2000s
full_ntl_data_2000s <- full_ntl_data |>
  filter(year >= 2000 & year < 2010)
t.test(full_ntl_data_2000s$median_income_2020_adj, full_ntl_data_2000s$mean_income_2020_adj)

# 2010s
full_ntl_data_2010s <- full_ntl_data |>
  filter(year >= 2010 & year < 2020)
t.test(full_ntl_data_2010s$median_income_2020_adj, full_ntl_data_2010s$mean_income_2020_adj)
```

Regression between percent union and party state average
```{r}
# 1984 to 2019
summary(lm(full_state_data$party_state_average~full_state_data$percent_union))

# 1980s
full_state_data_1980s <- full_state_data |>
  filter(year >= 1984 & year < 1990)
summary(lm(full_state_data_1980s$party_state_average~full_state_data_1980s$percent_union))

# 1990s
full_state_data_1990s <- full_state_data |>
  filter(year >= 1990 & year < 2000)
summary(lm(full_state_data_1990s$party_state_average~full_state_data_1990s$percent_union))

# 20000s
full_state_data_2000s <- full_state_data |>
  filter(year >= 2000 & year < 2010)
summary(lm(full_state_data_2000s$party_state_average~full_state_data_2000s$percent_union))

# 2010s
full_state_data_2010s <- full_state_data |>
  filter(year >= 2010 & year < 2020)
summary(lm(full_state_data_2010s$party_state_average~full_state_data_2010s$percent_union))
```
