...
States_by_union <- per_union_wage_data |> # use created table here
  ggplot(aes(x = year)) +
...
States_by_union
ggsave("../Documents/Graph_Images/States_by_union.png", width=7.29, height=5.41)
rm(per_union_wage_data, States_by_union)
```
...
States_by_CB <- per_collective_wage_data |> # use created table here
  ggplot(aes(x = year)) +
  ...
  labs(title = "States by % of Labor Market Covered by a Collective Bargaining Agreement",
       ...
       color = "Selected States")
States_by_CB
ggsave("../Documents/Graph_Images/States_by_CB.png", width=7.29, height=5.41)
rm(per_collective_wage_data, States_by_CB)
```
...
Income_by_year
ggsave("../Documents/Graph_Images/Income_by_year.png", width=7.29, height=5.41)
rm(Income_by_year)
```

```{r}
Income_by_year_adj <- full_ntl_data |> 
  filter(year >= 1984) |> 
  ggplot(aes(x = year)) +
...
Income_by_year_adj
ggsave("../Documents/Graph_Images/Income_by_year_adj.png", width=7.29, height=5.41)
rm(Income_by_year_adj)
```
...
Min_wage_by_state <- state_minimum_wage_data |> # use created table here
  ggplot(aes(x = year)) +
...
Min_wage_by_state
ggsave("../Documents/Graph_Images/Min_wage_by_state.png", width=7.29, height=5.41)
rm(Min_wage_by_state)
...
Eff_wage_by_state <- effective_minimum_wage_data |> 
  ggplot(aes(x = year)) +
...
Eff_wage_by_state
ggsave("../Documents/Graph_Images/Eff_wage_by_state.png", width=7.29, height=5.41)
rm(Eff_wage_by_state)
...
House_by_year <- plot_usmap(data = map_data, values = "party_state_average", color = "black") +
...
House_by_year
ggsave("../Documents/Graph_Images/House_by_year.png", width=7.29, height=5.41)
rm(map_data, House_by_year)
```
...
```{r}
Union_wage_by_year <- full_ntl_data |> 
  ggplot(aes(x = year)) +
...
Union_wage_by_year
ggsave("../Documents/Graph_Images/Union_wage_by_year.png", width=7.29, height=5.41)
rm(Union_wage_by_year)
```
...
```{r}
Union_wage_by_decade <- full_ntl_data |> 
...
Union_wage_by_decade
ggsave("../Documents/Graph_Images/Union_wage_by_decade.png", width=7.29, height=5.41)
rm(Union_wage_by_decade)
```
...
CB_by_decade1
ggsave("../Documents/Graph_Images/CB_by_decade1.png", width=7.29, height=5.41)
rm(CB_by_decade1)
```

```{r}
#| message: FALSE
CB_by_decade2 <- full_state_data |> 
  filter(year %% 4==0) |>
  ggplot(aes(x = percent_collective_bargain, y = party_state_average)) +
...
CB_by_decade2
ggsave("../Documents/Graph_Images/CB_by_decade2.png", width=7.29, height=5.41)
rm(CB_by_decade2)
```
...
### 2.4.1 T-test between union and non union wages
...
rm(full_ntl_data_1980s, full_ntl_data_2010s)
```

### 2.4.2 Linear regression of percent collective bargaining and party state average
...
rm(CB_party_temp, x, regression_decade_region)
```

#### 2.4.2.1 Graph of regression estimate by decade and region

```{r}
Regression_plot <- ggplot(CB_party_regression, aes(x = Decade, y = Estimate, fill = Region)) +
  ...
  labs(title = "Linear Regression Estimate by Decade",
       subtitle = "Model of Percent Collective Bargaining vs Proportion of House Delegation That is Democratic")

Regression_plot
ggsave("../Documents/Graph_Images/Regression_plot.png", width=7.29, height=5.41)
rm(Regression_plot, CB_party_regression, decade_region)
```
